...
// Then use Bob's token as JABEARRI_TOKEN_B when running the main test.
...
async function setup() {
  ...
  const tokens = {};
  for (const user of [
    { email: 'alice-jabearri@test.com', password: 'Alice1234!' },
    { email: 'bob-jabearri@test.com',   password: 'Bob1234!'   },
  ]) {
  ...
  const bobToken = tokens['bob-jabearri@test.com'];
  if (bobToken) {
    console.log('\n  ── Bob\'s token (for JABEARRI_TOKEN_B) ──────────────');
